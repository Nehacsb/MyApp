  ...
  useEffect(() => {
    const fetchRides = async () => {
      try {
        ...
        const createdResponse = await axios.get('http://web-production-de29.up.railway.app/api/rides', {
          params: { email: user.email },
          headers: { 'Content-Type': 'application/json' }
        });
        console.log('Fetched Created Rides:', createdResponse.data); // Debugging
        // Fetch rides requested by user
        const requestedResponse = await axios.get('http://web-production-de29.up.railway.app/api/request/requests', {
