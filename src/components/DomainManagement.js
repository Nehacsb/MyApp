...
const DomainManagement = () => {
  ...
  useEffect(() => {
    const fetchDomains = async () => {
      setIsLoading(true);
      try {
        const response = await fetch("http://192.168.225.30:5000/api/admin/authorized_domain");
  ...
  const addDomain = async () => {
    ...
    try {
      const response = await fetch("http://192.168.225.30:5000/api/admin/authorize_domain", {
        method: "POST",
  ...
  const removeDomain = async (domainToRemove) => {
    try {
      const response = await fetch("http://192.168.225.30:5000/api/admin/remove_domain", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
