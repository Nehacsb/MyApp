...
const PendingRequests = ({ onBack }) => {
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const ridesResponse = await axios.get('http://192.168.225.30:5000/api/rides', {
      ...
      const requestsResponse = await axios.get('http://192.168.225.30:5000/api/request/requests', {
        params: { 
  ...
  const handleRequestAction = async (requestId, action) => {
    try {
      ...
      await axios.patch(`http://192.168.225.30:5000/api/request/requests/${requestId}`, {
        status: action
      });
      ...
      if (action === 'accepted') {
        ...
        await axios.patch(`http://192.168.225.30:5000/api/request/${request.ride._id}/add-passenger`, {
